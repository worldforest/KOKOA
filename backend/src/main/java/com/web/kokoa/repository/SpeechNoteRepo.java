...
public interface SpeechNoteRepo extends JpaRepository<speechnote,String> {
    ...
    @Query(value = "select id, subtitleid, userid, videoid, engsubtitleid from speechnote where userid =:userid"
            ,countQuery="select count(id) from speechnote where userid =:userid",nativeQuery = true)
